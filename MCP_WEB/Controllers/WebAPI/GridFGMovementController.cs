...
namespace MCP_WEB.Controllers.WebAPI
{
    ...
    //[ApiController]
    public class GridFGMovementController : Controller
    {
        private NittanDBcontext _context;
        public GridFGMovementController(NittanDBcontext context)
        {
        ...
        {
            var fc = _context.WeeklyPlan.Distinct();
        ...
        [HttpGet]
        public IActionResult Get_data(DataSourceLoadOptions loadOptions, string fromdate, string todate)
        {
        ...
        [HttpPost]
        public IActionResult Filter(DataSourceLoadOptions loadOptions,string fromdate, string todate)
        {            
            DataTable dt = new DataTable();
            var msgj = "";
            if(fromdate == null) { fromdate = "2019-03-1"; }
        ...
        public IActionResult Print(string RowNumber, string fromdate, string todate)
        {
            ...
            var Log_Select_Print = _context.Log_Select_Print.Where(w => w.opt == "PrintFGMovement");

           
            var flag = false;
            flag = LogPrint.Log_Print(RowNumber, "PrintFGMovement", token, username, _context);
            //return View(pdr);
            return Json(flag);
        }
