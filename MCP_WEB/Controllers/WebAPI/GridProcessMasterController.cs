...
namespace MCP_WEB.Controllers.WebAPI {
    ...
    public class GridProcessMasterController : Controller {

        private NittanDBcontext _context;

        public GridProcessMasterController(NittanDBcontext context) {
        ...
        {            
            ...
            if (check_SysemProcessFLag.SysemProcessFLag == "Y")
            {
                ...
                model.ProcessName = _values["ProcessName"].ToString();
                model.TransDate = DateTime.Now;
                model.ModifyBy = GetUserName();
                _context.SaveChanges();
            }

            if (check_SysemProcessFLag.SysemProcessFLag == "N")
            {
                var checkedit = _context.WoRouting.Where(w => w.ProcessCode == key).ToList();
                if (checkedit.Count > 0)
                {
                    var model = _context.m_ProcessMaster.FirstOrDefault(item => item.ProcessCode == key);
                    if (model == null)
                        return StatusCode(409, "ProcessMaster not found");
                ...
                else
                {
                    ...
                    model.ProcessName = _values["ProcessName"].ToString();
                    model.ProcessCode = _values["ProcessCode"].ToString();
                    model.TransDate = DateTime.Now;
                    model.ModifyBy = GetUserName();                    
                    _context.SaveChanges();
