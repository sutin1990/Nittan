...
{
    public class PrintProductionDailyReport2Controller : Controller
    {
        private NittanDBcontext _context;
        public PrintProductionDailyReport2Controller(NittanDBcontext context)
        {
            _context = context;
        }
        ...
        public async Task<ActionResult>Index(string fromdate, string todate, string[] itemprocess)
        {
            ...
            try
            {
                using (var cmd = _context.Database.GetDbConnection().CreateCommand())
                {
                    ...
                    cmd.Parameters.Add(new SqlParameter("@FCode", SqlDbType.NVarChar) { Value = selectfcode });
